 ...
 *                 \_/ \_/ \_/     \_/ \_/ \_/ \_/ \_/
 *
 *   - Rename the prefix name of this class!
 *
 *   - Visit the plugin's homepage:
 *
 *   - http://tukod.com/to-code-projects/
 *
 ***********************************************************************
 *    ___           ___           ___           ___           ___      *
 *   /\  \         /\__\         /\__\         /\  \         /\  \     *
 ...
 * @author TuKod Team (TuKod.Com)
 * @version 2.0, April 2012
 */
class TuKod_MultiSite_Site_Names_TuKod_Plugin_Framework {
	...
	function Initialize($pluginInfo, $pluginDefaultOptions) {
		...
		// Initialize plugin options for networks
		if ( method_exists($this, 'network_options_page') ) {
			$this->ini_or_update_network_options();
		} else {
		...
		if ( 'site' == $this->g_info['action'] ) {
			$this->RegisterPluginOptionsPage();
		}

		// Register plugin options page for network
		if ( 'network' == $this->g_info['action'] ) {
			$this->register_network_options_page();
		}
	}
 	...
 	/**
	 * Register network options page
	 */
  	function register_network_options_page() {	// Add a menu item to the "Settings" area in the WP Network administration
//		add_action('admin_menu', array(&$this, 'add_action_admin_menu_PluginMenuItem'));
		if ( function_exists( 'add_action' ) ) {
			add_action( 'network_admin_menu', array(
					 &$this,
					'network_admin_plugin_menu'
				)
			);
		}
	}

	public function network_admin_plugin_menu() {
...
//		add_submenu_page( 'settings.php',			 __( 'TuKod Multi Sites', 'tukod-networks' ),	 __( 'TuKod MultiSites', 'tukod-networks' ),	 'manage_options',		 'tukod-multisitess',							 array( &$this, 'network_options_page' ) );
//		add_submenu_page( 'settings.php', $this->g_info['Name'], $this->g_info['nav_name'],	 'manage_options', basename($this->g_info['PluginFile']), array( &$this, 'network_options_page' ) );
		add_submenu_page( $this->g_info[ 'settings_php' ], $this->g_info[ 'Name' ], $this->g_info[ 'nav_name' ], 'manage_options', basename( $this->g_info[ 'PluginFile' ] ), array( &$this, 'network_options_page' ) );

		add_filter( 'network_admin_plugin_action_links_' . plugin_basename($this->g_info['PluginFile']), array(&$this, 'add_filter_plugin_action_links') );
//Feed Us!		add_filter( 'network_admin_plugin_action_links_' . plugin_basename($this->g_info['PluginFile']), array(&$this, 'add_filter_food_action_links') );
	}

	function add_filter_food_action_links($links) {
		$settings_link = '<a href="http://tukod.com/donate-to-tukod/feed-us-tukod/">' . __('Feed Us!') . '</a>';
...
//add_filter( 'network_admin_plugin_action_links_' . plugin_basename($this->g_info['PluginFile']), array(&$this, 'add_filter_plugin_action_links') );
	 ...
	 * Register plugin options page
	 */
  	function RegisterPluginOptionsPage() {
		// Add a menu item to the "Settings" area in the WP Site administration
		add_action('admin_menu', array(&$this, 'add_action_admin_menu_PluginMenuItem'));
	}
	function add_action_admin_menu_PluginMenuItem() {
		// Adding Options Page
	...
	function PrepareStandardSidebar() {
		$this->AddContentSidebar( __('Plugin',$this->g_info['ShortName']), '
			<ul>
				<li><a class="lwp" href="'. $this->g_info['PluginURI'] .  '">'.__('Plugin Homepage',$this->g_info['ShortName']).'</a></li>
				<li><a class="lwp" href="'. $this->g_info['SupportURI'] .  '">'.__('WordPress Support',$this->g_info['ShortName']).'</a></li>
				<li><a class="lwp" href="http://tukod.com/to-code-projects/tukod-wordpress-projects/">'.__('Other TuKod Plugins For WordPress',$this->g_info['ShortName']).'</a></li>
			</ul>
			');
		$this->AddContentSidebar( __('Do you like this plugin?',$this->g_info['ShortName']), '
			<p style="font-size:8pt;">'. __('We spend a huge amount of time on the plugins we\'ve written for WordPress. Any donation, to help buy us food, would be greatly appreciated.',$this->g_info['ShortName']).'</p>
			<ul>
				<li><a class="lpaypal" href="http://tukod.com/donate-to-tukod/paypal/">'.__('Donate via PayPal',$this->g_info['ShortName']).'</a></li>
				<li><a class="lamazon" href="http://tukod.com/donate-to-tukod/amazon/">'.__('My Amazon Wish List',$this->g_info['ShortName']).'</a></li>
			</ul>
			');
	}
	 ...
	 * Initialize and (if required) save/update the options.
	 */
	function ini_or_update_network_options() {
		...
		$isdeleted = false;
		if ( isset($_POST['delete-settings-'.$this->g_info['ShortName']]) ) {
			delete_site_option($this->g_info['OptionName']);
			$isdeleted = true;
		}
		...
		# Initialize options
		##########################
		$this->g_opt = get_site_option( $this->g_info[ 'OptionName' ]);
		...
		if ( (!$isdeleted) && ($this->g_opt['pluginversion'] != $this->g_info['Version']) ) {
			...
			$savedoptnameArr = array_reverse($savedoptnameArr); // newest option first
			foreach ($savedoptnameArr as $loopval) {
				if ( get_site_option($loopval) != false ) {
					$opttemp = get_site_option($loopval);
					if ( (is_array($opttemp)) && (!empty($opttemp)) ) {
						foreach ($opttemp as $lpOptionName => $lpOptionValue) {
							if ($lpOptionName != 'pluginversion') {
								$this->g_opt[$lpOptionName] = $lpOptionValue;
						...
						// We save the options here.
						$this->g_opt['pluginversion'] = $this->g_info['Version'];
						add_site_option($this->g_info['OptionName'], $this->g_opt); 	// adds option to table if it does not exist.
						update_site_option($this->g_info['OptionName'], $this->g_opt);	// we save option since add_site_option does nothing if option already exists
						break;
					}
				}
			}
		...
		if ( isset($_POST['update-options-'.$this->g_info['ShortName']]) ) {
			...
			// Update Options in the database
			add_site_option($this->g_info['OptionName'], $optionsToBeSaved); 	// adds option to table if it does not exist.
			update_site_option($this->g_info['OptionName'], $optionsToBeSaved);	// we save option since add_site_option does nothing if option already exists
			// Update Options in the class
			$this->g_opt = $optionsToBeSaved;
		}
	}
	...
	function GetGeneratedOptionsPage() {
		...
		if ( isset($_POST['delete-settings-'.$this->g_info['ShortName']]) || isset($_POST['update-options-'.$this->g_info['ShortName']]) ) {
			check_admin_referer($this->g_info['Name']); // LM
		}
		...
		<style type="text/css">
			...
			pre { font-size: 10px; line-height: 1.4em; background: #dfdfdf url(<?php echo $this->GetBase64IconURL('bg-header-gray.png'); ?>) repeat-x left top; }
			code { background: #dfdfdf url(<?php echo $this->GetBase64IconURL('bg-header-gray.png'); ?>) repeat-x left top; }
		</style>
	...
	function GetPluginOptionsURL() {
		...
		if ( method_exists($this, 'network_options_page') ) {
			return $adminurl.'network/' . $this->g_info[ 'settings_php' ] .'?page=' . basename($this->g_info['PluginFile']);
		}
...
} // end class ..._TuKod_Plugin_Framework
